...
Version: 1.3
Description:
    Interactive tool to manually define a polygonal region over Greenland
    and generate a land mask stored in a netCDF file.

Update Note:
    - Added polygon counter at the bottom of the screen
    - Added option to display mask area
"""
...
# /!\ Replace the path to reference netcdf with correct one /!\ #
path = ""
exten = ""
file_path = "tile10.nc"
...
def on_click(event):
    ...
    if (event.button == 1) and (not(is_poly_closed)): # Left click: add vertex
        ...
        if event.key == "control": # Ctrl click to remove polygon
            ...
            status_label.config(text=f"Polygons: {len(plotgons)}")
...
status_label = tk.Label(root, text="Polygons : 0", font=("Segoe UI", 10))
